...
export function PublisherBookUploadForm({ bookId }: { bookId?: string } = {}) {
  ...
  const { suggestions, showSuggestions, search, setShowSuggestions, isLoading: isSearching } = useAuthorSearch();
  ...
  useEffect(() => {
    ...
    reset({
      ...
      author_id: editBook.author_user_id || '',
    });
    ...
    if (editBook.author_name && !editBook.author_user_id) {
      setAuthorSearchTerm(editBook.author_name);
    }
  ...
  const onSubmit = async (data: BookUploadFormData) => {
    ...
    if (!isEditMode && !selectedAuthorId && !authorSearchTerm.trim()) {
  ...
  const onSaveDraft = async (data: BookUploadFormData) => {
    ...
    // Optional author link while drafting
    if (data.author_id) {
      formData.append('author_user_id', data.author_id);
    } else if (authorSearchTerm.trim()) {
      formData.append('author_name', authorSearchTerm.trim());
    }
      ...
      {uploadSuccess && (
        <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl p-4">
          ...
          <div>
            ...
            <p className="text-green-700 text-sm">It will appear in your catalog once approved.</p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="relative md:col-span-2">
            <label className="block text-sm font-medium text-[#1A2A3A] mb-1">Author *</label>
            <input
              type="text"
            ...
            {selectedAuthorId && (
              <p className="text-xs text-green-600 mt-1 flex items-center gap-1">
                <CheckCircle size={12} /> Linked to existing author
              </p>
            )}
            {!selectedAuthorId && authorSearchTerm && !showSuggestions && authorSearchTerm.length > 2 && (
              <p className="text-xs text-[#4A5568] mt-1">Will create new author: "{authorSearchTerm}"</p>
            )}
          </div>
        </div>
      </div>
