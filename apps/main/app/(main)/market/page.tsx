    ...
    <div className="min-h-10 bg-[#FDFBF7]">
      <div className="relative bg-gradient-to-r from-[#2C3E50] to-[#1A2A3A] text-white overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-3">{t('market.heroTitle')}</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex justify-between items-center mb-6 gap-4 flex-wrap">
          <p className="text-sm text-[#4A5568]">
            {!isLoading && !isError && total > 0 && (
              t('market.showingRange', { start: rangeStart, end: rangeEnd, total })
            )}
            {!isLoading && !isError && total === 0 && (
              <span>{t('market.noMatch')}</span>
            )}
          </p>
          {hasActiveFilters && (
            <button
        ...
        {isError && (
          <div className="text-center py-16 bg-white rounded-2xl border border-[#E8E2D9]">
            ...
            <p className="text-[#4A5568]">{t('market.loadFailed')}</p>
            <button
              type="button"
      ...
      fallback={
        <div className="min-h-screen bg-[#FDFBF7] flex items-center justify-center">
          <div className="animate-pulse text-center">
            ...
            <p className="text-[#4A5568] mt-4">{t('market.loadingMarketplace')}</p>
          </div>
        </div>
      }
    >
